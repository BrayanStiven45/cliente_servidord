...
files = glob.glob("client_servidor_test_multiprocesing/results_*.csv")

# Ordenar por número de procesos (no alfabéticamente)

files = sorted(files, key=lambda x: int(x.split("_")[4].split(".")[0]))
...
for file in files:
    n_proc = int(file.split("_")[4].split(".")[0])
